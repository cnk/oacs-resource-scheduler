...
# 	@cvs-id			$Id: personnel-template.tcl,v 1.1.1.1 2006/09/13 01:29:58 nsadmin Exp $
#
...
if {![personnel::personnel_exists_p -personnel_id $personnel_id]} {
	ad_return_error "Error" "The personnel ID passed to this page is invalid.
...
db_1row inst_personnel_info {
	select	p.first_names	as first_name,
			...
			photo_height height,
			ip.bio,
	        decode(lower(c.name),'retired',c.name,'deceased',c.name,'') as retired_deceased_status
	  from	inst_personnel ip,
			persons p,
	        categories c
	 where	ip.personnel_id = :personnel_id
	   and	p.person_id	    = ip.personnel_id
	   and  ip.status_id    = c.category_id(+)
}
...
db_multirow url_info inst_personnel_url_info {
	...
	and		(u.url_type_id	= :med_pract_category_id
			 or :physician_p = 0)
} {
