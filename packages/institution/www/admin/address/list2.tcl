...
#									of addresses that should be	displayed.
...
#									addresses.
# @param	if_none_put			(optional) The text to use if there are no addresses.
#									If omitted, nothing will be displayed.
#
# @author			helsleya@cs.ucr.edu (AH)
# @creation-date	2004-02-05
# @cvs-id			$Id: list2.tcl,v 1.1.1.1 2006/09/13 01:29:59 nsadmin Exp $
#
...
set parent_url "[ad_conn package_url]admin/address/"

# setup the title of the first column of the table if none was passed in
if {![info exists list_title]} {
	set list_title "Address"
}

# 'address_action_url_exists_p' tells us whether the user is allowed to perform actions on the address.
#	An action URL is (typically) anything except 'view' or 'details'
set address_action_url_exists_p 0
...
db_multirow -extend {
	...
	# read_p --> user can see this address
	# All results from the query should be readable, but just in case...
	if {!$read_p} {
		continue
	} elseif {$allow_action_p(view)} {
		# url for showing details about a particular address
		set detail_url	"${parent_url}detail?[export_vars {address_id return_url}]"
		# this is not an "action URL" so don't set address_action_url_exists_p
	}

	# write_p --> user can edit this address
	if {$allow_action_p(edit) && $write_p}	{
		set edit_url "${parent_url}add-edit?[export_vars {address_id return_url}]"
		set action_url_exists_p				1
		set address_action_url_exists_p	1
	}

	# delete_p --> user can delete this address
	if {$allow_action_p(delete) && $delete_p}	{
		set delete_url "${parent_url}delete?[export_vars {address_id return_url}]"
		set action_url_exists_p				1
		set address_action_url_exists_p	1
	}

	# admin_p --> user can change the privileges on this address
	if {$allow_action_p(permit) && $admin_p}	{
		set permit_url "${subsite_url}permissions/one?[export_vars -override {{object_id $address_id} return_url}]"
		set action_url_exists_p				1
		set address_action_url_exists_p	1
	}
}
