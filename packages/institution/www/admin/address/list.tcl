...
db_multirow -extend {
	...
	if {!$read_p} {
		...
		set detail_url	"${parent_url}detail?[export_vars {address_id return_url step}]"
	...
	if {$allow_action_p(edit) && $write_p}	{
		set edit_url "${parent_url}add-edit?[export_vars {address_id return_url step}]"
	...
	if {$allow_action_p(delete) && $delete_p}	{
		set delete_url "${parent_url}delete?[export_vars {address_id return_url step}]"
		set action_url_exists_p				1
		set address_action_url_exists_p	1
	}
