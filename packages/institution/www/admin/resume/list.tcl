...
#									of resumes that should be	displayed.
...
#									resumes.
# @param	if_none_put			(optional) The text to use if there are no resumes.
...
set parent_url "[ad_conn package_url]admin/resume/"

# setup the title of the first column of the table if none was passed in
if {![info exists list_title]} {
	set list_title "Resume"
}

# 'resume_action_url_exists_p' tells us whether the user is allowed to perform actions on the resume.
#	An action URL is (typically) anything except 'view' or 'details'
set resume_action_url_exists_p		0
set resume_download_url_exists_p	0
...
db_multirow -extend {
	...
	# read_p --> user can see this resume
	# All results from the query should be readable, but just in case...
	if {!$read_p} {
		continue
	} elseif {$allow_action_p(view)} {
		# url for showing details about a particular resume
		set detail_url		"${parent_url}detail?[export_vars {resume_id return_url step}]"
		if {[exists_and_not_null content_length] && $content_length > 0} {
			set download_size					[ctrl::data_size -max_digits 4 -bytes $content_length]
			set download_url					"[ad_conn package_url]resume-download?[export_vars {resume_id}]"
			set resume_download_url_exists_p	1
		}
		# this is not an "action URL" so don't set resume_action_url_exists_p
	}

	# write_p --> user can edit this resume
	if {$allow_action_p(edit) && $write_p}	{
		set edit_url "${parent_url}add-edit?[export_vars {resume_id return_url step}]"
		set action_url_exists_p				1
		set resume_action_url_exists_p		1
	}

	# delete_p --> user can delete this resume
	if {$allow_action_p(delete) && $delete_p}	{
		set delete_url "${parent_url}delete?[export_vars {resume_id return_url step}]"
		set action_url_exists_p				1
		set resume_action_url_exists_p		1
	}

	# admin_p --> user can change the privileges on this resume
	if {$allow_action_p(permit) && $admin_p}	{
		set permit_url "${subsite_url}permissions/one?[export_vars -override {{object_id $resume_id} return_url}]"
		set action_url_exists_p				1
		set resume_action_url_exists_p		1
	}
}
