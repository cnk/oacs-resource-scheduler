...
#									of certifications that should be	displayed.
...
#									certifications.
# @param	if_none_put			(optional) The text to use if there are no certifications.
...
set parent_url "[ad_conn package_url]admin/certification/"

# setup the title of the first column of the table if none was passed in
if {![info exists list_title]} {
	set list_title "Certification"
}

# 'certification_action_url_exists_p' tells us whether the user is allowed to perform actions on the certification.
#	An action URL is (typically) anything except 'view' or 'details'
set certification_action_url_exists_p 0
...
db_multirow -extend {
	...
	# read_p --> user can see this certification
	# All results from the query should be readable, but just in case...
	if {!$read_p} {
		continue
	} elseif {$allow_action_p(view)} {
		# url for showing details about a particular certification
		set detail_url	"${parent_url}detail?[export_vars {certification_id return_url step}]"
		# this is not an "action URL" so don't set certification_action_url_exists_p
	}

	# write_p --> user can edit this certification
	if {$allow_action_p(edit) && $write_p}	{
		set edit_url "${parent_url}add-edit?[export_vars {certification_id return_url step}]"
		set action_url_exists_p				1
		set certification_action_url_exists_p	1
	}

	# delete_p --> user can delete this certification
	if {$allow_action_p(delete) && $delete_p}	{
		set delete_url "${parent_url}delete?[export_vars {certification_id return_url step}]"
		set action_url_exists_p				1
		set certification_action_url_exists_p	1
	}

	# admin_p --> user can change the privileges on this certification
	if {$allow_action_p(permit) && $admin_p}	{
		set permit_url "${subsite_url}permissions/one?[export_vars -override {{object_id $certification_id} return_url}]"
		set action_url_exists_p				1
		set certification_action_url_exists_p	1
	}
}
