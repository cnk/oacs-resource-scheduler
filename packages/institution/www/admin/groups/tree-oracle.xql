	...
	<fullquery name="group_tree">
	 <querytext>
		select	group_id,
				...
				(select name from categories where category_id = group_type_id) as group_type,
				(select count(*)
				   from inst_groups
				  where parent_group_id = g.group_id) as n_children,
				decode(acs_permission.permission_p(group_id, :user_id, 'read'),
						't', 1, 0) read_p,
				decode(acs_permission.permission_p(group_id, :user_id, 'write'),
						't', 1, 0) write_p,
				decode(acs_permission.permission_p(group_id, :user_id, 'create'),
						't', 1, 0) create_p,
				decode(acs_permission.permission_p(group_id, :user_id, 'delete'),
						't', 1, 0) delete_p,
				decode(acs_permission.permission_p(group_id, :user_id, 'admin'),
						't', 1, 0) admin_p
		  from	inst_groups	g
		 where	($depth_filter
				or	parent_group_id		in ([join $visible_expanded ","])
