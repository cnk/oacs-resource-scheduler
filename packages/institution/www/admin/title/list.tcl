# -*- tab-width: 4 -*-
#
# @param	items				The titles that should be listed.
...
#									should ever be displayed in the list unless they are
#									listed in the array with a 1.
...
#									titles.
# @param	if_none_put			(optional) The text to use if there are no titles.
...
set parent_url "[ad_conn package_url]admin/title/"
...
# 'title_action_url_exists_p' tells us whether the user is allowed to perform actions on the title.
#	An action URL is (typically) anything except 'view' or 'details'
set title_action_url_exists_p 0
...
db_multirow -extend {
	...
	# read_p --> user can see this title
	# All results from the query should be readable, but just in case...
	if {!$read_p} {
		continue
	} elseif {$allow_action_p(view)} {
		# url for showing details about a particular title
		set detail_url		"${parent_url}detail?[export_vars {gpm_title_id return_url step}]"
		set group_url		"${parent_url}../groups/detail?[export_vars {group_id}]"
		# this is not an "action URL" so don't set title_action_url_exists_p
	}

	# write_p --> user can edit this title
	if {$allow_action_p(edit) && $write_p}	{
		set edit_url			"${parent_url}add-edit?[export_vars {gpm_title_id return_url step}]"
		set action_url_exists_p			1
		set title_action_url_exists_p	1
	}

	# create_p --> user can add a title within this group
	if {$allow_action_p(add) && $create_p} {
		set create_url	"${parent_url}add-edit?[export_vars {personnel_id group_id step}]"
		set action_url_exists_p			1
		set title_action_url_exists_p	1
	}

	# delete_p --> user can delete this title
	if {$allow_action_p(delete) && $delete_p}	{
		set delete_url			"${parent_url}delete?[export_vars {gpm_title_id return_url step}]"
		set delete_all_url		"${parent_url}delete?[export_vars {acs_rel_id return_url step}]"
		set action_url_exists_p			1
		set title_action_url_exists_p	1
	}

	# admin_p --> user can change the privileges on this title
	if {$allow_action_p(permit) && $admin_p}	{
		set permit_url			"${subsite_url}permissions/one?[export_vars -override {{object_id $gpm_title_id} return_url}]"
		set action_url_exists_p			1
		set title_action_url_exists_p	1
	}
}
