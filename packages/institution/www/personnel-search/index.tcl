...
################################################################################
ad_form -name search -export [list select_to select_into return_url] -action "?[export_vars {force_reload}]" -form {
	...
	{search:text(submit)						{label "Search"}				{html {onClick "reSubmitInitial(false); return true;"}}}
	...
	set count_sql "
		select	count(inst_personnel.personnel_id)
			$filter_and_sort_sql
	"
...
#	ad_script_abort
	...
	db_multirow -extend {
		...
		# //TODO// add in ordering of titles? performance might be too much
		#	of an issue though...
		#	could reformat the query to have multiple rows per person and use
		#		ADP group tags, but it seems bandwidth/latency between
		#		app-server and db-server are already issues
