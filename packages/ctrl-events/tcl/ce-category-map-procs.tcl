...
ad_library {
    ...
    @author SH (shhong@mednet.ucla.edu)
    @author AK (avni@avni.net) - Ported to Postgres

    @creation-date  Feb 24 2006
    @modification-date Sept 13 2008
    @cvs-id $Id$ 
}

namespace eval ctrl_event::category {}

ad_proc -public ctrl_event::category::associate {
    {-event_id:required}
...
ad_proc -public ctrl_event::category::disassociate {
    {-event_id:required}
    {-category_id:required}
} {
...
ad_proc -public ctrl_event::category::current {
   {-event_id:required}
} {
   set result ""
...
ad_proc -public ctrl_event::category::category_csv {
   {-event_id:required}
} {
   set result ""
   db_foreach names {} {
...
ad_proc -public ctrl_event::category::update {
    {-event_id:required}
    {-category_widget ""}
} {
   ...
   set error_p 0
   db_transaction {
      set cur_category_list [ctrl_event::category::current -event_id $event_id]
      if {[llength $category_widget] > 0 } {
          foreach cur_category_id $cur_category_list {
             if {[lsearch -glob $category_widget $cur_category_id] == -1} {
                ctrl_event::category::disassociate -event_id $event_id -category_id $cur_category_id
             }
          } 
          foreach new_category_id $category_widget {
             if {[lsearch -glob $cur_category_list $new_category_id] == -1} {
                ctrl_event::category::associate -event_id $event_id -category_id $new_category_id
             } 
          }
      } else {
          foreach cur_category_id $cur_category_list {
